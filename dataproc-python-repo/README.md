...
Для создания среды Python с нужными библиотеками используется временный кластер Data Proc, имеющий доступ в интернет для скачивания дистрибутивов из репозиториев пакетов `conda`. Установка необходимых пакетов выполняется в интерактивном режиме, что позволяет уточнить состав необходимых пакетов и их версий и разобраться с возникающими проблемами установки.
...
Точный состав использованных (успешных) команд по установке пакетов желательно зафиксировать для последующего использования в скриптах инициализации.
...
4. Применяем патч для Zeppelin 0.9.0 из-за несовместимости с обновлениями Python 3.8 (только для Data Proc 2.0).
5. Используем подготовленные скрипты инициализации для создания продуктивных кластеров Data Proc.

Процедура создания локального репозитория ниже приведена для менеджера пакетов `conda`. Если, в дополнение к `conda`, в окружение Python необходимо устанавливать пакеты с помощью инструмента `pip` (не рекомендовано), то для создания частичного зеркала может использоваться [специальный инструмент python-pypi-mirror](https://pypi.org/project/python-pypi-mirror/). Локальное зеркало пакетов `pip` далее может быть использовано на рабочих кластерах Data Proc в скриптах инициализации, аналогично описанному выше варианту для `conda`.
...
### 1.3 Скрипт инициализации с использованием локального репозитория
...
Предлагаемый пример скрипта инициализации [`init-conda-basic.sh`](init-conda-basic.sh) принимает путь к локальному репозиторию в качестве аргумента.

### 1.4 Патч для Zeppelin 0.9.0
...
Предлагаемый пример скрипта инициализации [`init-patch-zeppelin090.sh`](init-patch-zeppelin090.sh) принимает полный путь к исправленному файлу `zeppelin_python.py` в качестве аргумента (например, `s3a://dproc-repo/repos/zeppelin_python.py`), и заменяет старые копии этого файла на узле кластера Data Proc.

### 1.5 Пример создания кластера Data Proc

Создание продуктивных кластеров Data Proc с необходимыми версиями библиотек Python обеспечивается использованием описанных выше скриптов инициализации. Пример настроек для создания кластера можно посмотреть в скрипте [dp-pyenv-cluster-basic.sh](dp-pyenv-cluster-basic.sh).

## Вариант 2 - Образ среды

Общая логика:
1. Готовим файл образа с подготовленными окружением `/opt/conda` в сжатом формате `squashfs`.
2. Размещаем подготовленный файл образа в бакете объектного хранилища.
