...
yc dataproc cluster create ${YC_CLUSTER} \
  ...
  --subcluster name="static",role='computenode',resource-preset='m3-c8-m64',preemptible=true,disk-type='network-ssd-nonreplicated',disk-size=186,hosts-count=1,max-hosts-count=1,subnet-name=${YC_SUBNET},autoscaling-decommission-timeout=3600 \
  --subcluster name="dynamic",role='computenode',resource-preset='m3-c8-m64',preemptible=true,disk-type='network-ssd-nonreplicated',disk-size=186,hosts-count=1,max-hosts-count=6,subnet-name=${YC_SUBNET},autoscaling-decommission-timeout=3600 \
