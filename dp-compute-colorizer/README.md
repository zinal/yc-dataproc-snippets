...
Сопоставление ресурсов кластерам Data Proc в этом примере запроса выполняется через справочник `dp_cluster_dict`, формируемый путём выгрузки в объектное хранилище результатов сканирования структуры кластеров Data Proc. Затем осуществляется привязка к идентификаторам бизнес-проектов, которая настроена через метки `project_id` на объекты кластеров Data Proc.
