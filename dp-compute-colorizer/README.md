...
* бессерверная база данных YDB для хранения оперативных данных о структуре вычислительных ресурсов кластеров Data Proc;
* облачная функция для сканирования структуры ресурсов кластеров Data Proc (одна на каждую папку Облака);
...
* секрет в сервисе Yandex Lockbox для хранения статического ключа доступа к объектному хранилищу.
...
Пример команд для создания необходимых облачных функций Yandex Cloud приведён в файле `install.sh`. Пример команд для удаления бессерверных функций есть в файле `cleanup.sh`.

Идентификационная информация бакета объектного хранилища и бессерверной базы данных YDB при установке стандартными скриптами указывается в файле `options.sh`.
...
Пользовательские метки доступны в данных биллинга в виде дополнительных колонок формата `label.user_labels.<ИмяМетки>`, и могут использоваться в запросах Yandex Query над данными биллинга.
